"""
GPT-2 Small SST-2 Analysis with MLP-out SAE (SAELens)
Hook: blocks.N.hook_mlp_out
SAE: jbloom/GPT2-Small-OAI-v5-32k-mlp-out-SAEs (32k features)
...
with open(os.path.join(RESULTS_DIR, 'baseline_results.txt'), 'w') as f:
    f.write("="*70 + "\n")
    f.write("BASELINE - ZERO-SHOT PROMPTING (GPT-2 Small, MLP-out hook)\n")
    f.write("="*70 + "\n\n")
...
# ======================================================================
# STEP 2: Layer-wise Analysis (Probes + MLP-out SAEs)
# ======================================================================
print("\n" + "="*70)
print("STEP 2: LAYER-WISE ANALYSIS (MLP-out SAEs)")
print("="*70)
...
SAE_RELEASE = "jbloom/GPT2-Small-OAI-v5-32k-mlp-out-SAEs"
...
for layer_num in range(model.cfg.n_layers):
    ...
    # --------------------------------------------------------------
    # Part B: MLP-out SAE feature analysis (MAX POOLING)
    # --------------------------------------------------------------
    print("\n--- Part B: MLP-out SAE Feature Analysis ---")

    sae_id = f"v5_32k_layer_{layer_num}"

    try:
        print(f"Loading MLP-out SAE: {SAE_RELEASE}, {sae_id}")
    ...
    with open(os.path.join(RESULTS_DIR, f"layer_{layer_num}_mlp_analysis.txt"), "w") as f:
        f.write("="*70 + "\n")
        f.write(f"LAYER {layer_num} - MLP-OUT SAE ANALYSIS (GPT-2 Small)\n")
        f.write(f"Pooling: Max | SAE features: 32k\n")
        f.write("="*70 + "\n\n")
...
# ======================================================================
print("\n" + "="*70)
print("FINAL SUMMARY (MLP-out SAE, Max Pooling)")
print("="*70)
...
with open(os.path.join(RESULTS_DIR, "final_summary_mlp.txt"), "w") as f:
    f.write("="*70 + "\n")
    f.write("FINAL SUMMARY - GPT-2 Small MLP-OUT SAE (SST-2)\n")
    f.write("="*70 + "\n\n")
    f.write("SAE: jbloom/GPT2-Small-OAI-v5-32k-mlp-out-SAEs\n")
    f.write("Hook: blocks.N.hook_mlp_out\n")
    f.write("Features: 32k per layer | Pooling: Max\n\n")
