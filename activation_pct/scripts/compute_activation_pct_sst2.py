# -*- coding: utf-8 -*-
"""Per-sample activation percentage for DeepSeek on SST-2
...
# Use validation + some train to get 1000 samples
sentences = [ex["sentence"] for ex in val_dataset]
labels = [ex["label"] for ex in val_dataset]
...
LAYER = 14  # best probing layer for SST-2
...
print(f"{'='*70}")
print(f"Task: SST-2 (Sentiment Analysis)")
...
plt.tight_layout()
out_png = os.path.join(SCRIPT_DIR, "sst2_deepseek_activation_pct.png")
out_pdf = os.path.join(SCRIPT_DIR, "sst2_deepseek_activation_pct.pdf")
...
# ── Save text summary ─────────────────────────────────────────
txt_path = os.path.join(SCRIPT_DIR, "sst2_deepseek_activation_pct.txt")
with open(txt_path, 'w') as f:
    ...
    f.write(f"Task: SST-2 (Sentiment Analysis)\n")
