# -*- coding: utf-8 -*-
"""Per-sample activation percentage for Qwen3-0.6B on all 3 tasks
SAE: mwhanna-qwen3-0.6b-transcoders-lowl0 (Transcoder)
Uses HF Transformers + SAELens
"""
...
TASKS = {
    ...
    "mrpc": {"layer": 14, "name": "MRPC (Paraphrase)"},
    "mt":   {"layer": 15, "name": "MT (Europarl)"},
}
...
def compute_per_sample(task_key, layer, sentences):
    ...
    with torch.no_grad():
        for idx in range(len(sentences)):
            layer_h = get_layer_rep(sentences[idx], layer)
            sae_feats = sae.encode(layer_h)
            pooled = sae_feats.mean(dim=1)[0].to(torch.float32).cpu().numpy().flatten()
            ac = (pooled > 0).sum()
...
def report_and_plot(task_key, task_name, layer, all_pcts, all_counts,
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS: {task_name} | Qwen3-0.6B | Layer {layer} | d_sae={num_latents}")
    print(f"{'='*70}")
    ...
    ax.set_title(f'(A) Per-Sample Activation %\nQwen3-0.6B, Layer {layer}, {task_name}'); ax.legend()
    ...
    plt.tight_layout()
    base = os.path.join(SCRIPT_DIR, f"{task_key}_qwen3_activation_pct")
