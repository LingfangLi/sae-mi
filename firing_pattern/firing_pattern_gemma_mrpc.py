# -*- coding: utf-8 -*-
"""Firing pattern visualization for Gemma-2-2B on MRPC (paraphrase detection).
...
SAE:   gemma-scope-2b-pt-att-canonical (16k, attn hook_z)
Layer: 12  (mean-pool probing-best on MRPC, from
       saes-mi-repo/mean-pooling/mrpc/gemma2-2b/results_att/final_summary.txt:
       L12 = 75.49%)
Pool:  Mean
"""
...
OUT_DIR   = os.path.join(SCRIPT_DIR, "gemma_mrpc")
...
# ── Pair combine (same as Stage-1 Gemma MRPC script) ─────────
EOS = "<|endoftext|>"
def combine_pair(s1, s2):
    return f"{s1} {EOS} {s2}"

# ── Zero-shot paraphrase prediction (same as Stage-1 Gemma MRPC script) ──
...
Answer (yes or no):"""
    ...
    if "no" in generated_text:
        return 0
    yes_id = model.to_tokens(" yes", prepend_bos=False)[0, 0]
    no_id  = model.to_tokens(" no",  prepend_bos=False)[0, 0]
...
fig, y_avg_n, n_avg_n = plot_avg_features(
    ...
    yes_plot_acts, no_plot_acts, MODEL_NAME)
out = os.path.join(OUT_DIR, "firing_pattern_gemma_mrpc_no_avg5.png")
fig.savefig(out, dpi=150, bbox_inches='tight')
fig.savefig(out.replace('.png', '.pdf'), bbox_inches='tight')
plt.close()
print(f"Saved: {out}")
...
for tag, feat_ids, colname in [
    ...
    csv_path = os.path.join(OUT_DIR, f"firing_values_gemma_mrpc_{tag}_avg5.csv")
...
for tag, sel, plt_lst in [("yes", yes_select, yes_plot),
                           ("no",  no_select,  no_plot)]:
    csv_path = os.path.join(OUT_DIR, f"filtered_pairs_gemma_mrpc_{tag}.csv")
    with open(csv_path, 'w', newline='') as f:
        w = csv.writer(f)
...
print(f"\n{'='*70}")
print(f"VALUES: Avg Top-5 Paraphrase (semantic-sim) (Gemma-2-2B, MRPC, L{LAYER})")
...
print(f"\nVALUES: Avg Top-5 Non-paraphrase (Gemma-2-2B, MRPC, L{LAYER})")
