# -*- coding: utf-8 -*-
"""Firing pattern visualization for Qwen3-0.6B on SST-2

Model: Qwen3-0.6B-Base (HF Transformers)
SAE: mwhanna-qwen3-0.6b-transcoders-lowl0 (Transcoder), mean pooling
Layer: 12
Zero-shot: same prompt as qwen3_0_6b_pretrained.py
"""
...
def predict_sentiment_zeroshot(sentence):
    """Same method as qwen3_0_6b_pretrained.py"""
...
def get_sae_activation(sentence):
    ...
    sae_feats = sae.encode(layer_h)                  # (1, seq, d_sae)
...
out = os.path.join(SCRIPT_DIR, "firing_pattern_qwen3_pos_avg5.png")
...
out = os.path.join(SCRIPT_DIR, "firing_pattern_qwen3_neg_avg5.png")
fig.savefig(out, dpi=150, bbox_inches='tight')
fig.savefig(out.replace('.png', '.pdf'), bbox_inches='tight')
plt.close()
print(f"Saved: {out}")
...
for tag, feat_ids, label in [("pos", top_pos_idx[:5], "avg_top5_pos_features"),
    ...
    csv_path = os.path.join(SCRIPT_DIR, f"firing_values_qwen3_{tag}_avg5.csv")
...
for tag, sents_sel, sents_plt in [("pos", pos_select, pos_plot), ("neg", neg_select, neg_plot)]:
    csv_path = os.path.join(SCRIPT_DIR, f"filtered_sentences_qwen3_{tag}.csv")
    with open(csv_path, 'w', newline='') as f:
        w = csv.writer(f)
...
print(f"\n{'='*70}")
print(f"VALUES: Avg Top-5 Positive Features (Qwen3-0.6B)")
print(f"Features: {list(top_pos_idx[:5])}")
print(f"Pos mean={pos_avg.mean():.4f}, Neg mean={neg_avg.mean():.4f}")
print(f"\nVALUES: Avg Top-5 Negative Features (Qwen3-0.6B)")
