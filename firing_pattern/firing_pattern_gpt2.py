# -*- coding: utf-8 -*-
"""Firing pattern visualization for GPT-2 Small on SST-2

Model: GPT-2 Small (TransformerLens)
SAE: gpt2-small-hook-z-kk (attention hook_z), mean pooling
Layer: 7
Zero-shot: same prompt as pretrained_gpt2_sae_saelens.py
"""
...
def predict_sentiment_zeroshot(sentence):
    """Same method as pretrained_gpt2_sae_saelens.py"""
...
out = os.path.join(SCRIPT_DIR, "firing_pattern_gpt2_pos_avg5.png")
...
out = os.path.join(SCRIPT_DIR, "firing_pattern_gpt2_neg_avg5.png")
fig.savefig(out, dpi=150, bbox_inches='tight')
fig.savefig(out.replace('.png', '.pdf'), bbox_inches='tight')
plt.close()
print(f"Saved: {out}")
...
for tag, feat_ids, label in [("pos", top_pos_idx[:5], "avg_top5_pos_features"),
    ...
    csv_path = os.path.join(SCRIPT_DIR, f"firing_values_gpt2_{tag}_avg5.csv")
...
for tag, sents_sel, sents_plt in [("pos", pos_select, pos_plot), ("neg", neg_select, neg_plot)]:
    csv_path = os.path.join(SCRIPT_DIR, f"filtered_sentences_gpt2_{tag}.csv")
    with open(csv_path, 'w', newline='') as f:
        w = csv.writer(f)
