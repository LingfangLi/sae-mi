# -*- coding: utf-8 -*-
"""Firing pattern visualization for DeepSeek-R1-Distill-Qwen-1.5B on MRPC.
...
Model: DeepSeek-R1-Distill-Qwen-1.5B (HF Transformers)
SAE:   EleutherAI/sae-DeepSeek-R1-Distill-Qwen-1.5B-65k (sparsify, MLP hookpoint)
Layer: 15  (mean-pool MLP probing-best on MRPC, from
       saes-mi-repo/mean-pooling/mrpc/qwen25-1.5b/results_mlp/final_summary_deepseek.txt:
       L15 = 73.04%)
Pool:  Mean
"""
...
OUT_DIR   = os.path.join(SCRIPT_DIR, "deepseek_mrpc")
...
# ── Zero-shot paraphrase prediction (same as Stage-1 DeepSeek MRPC script) ──
def predict_paraphrase_zeroshot(s1, s2):
    sentence = combine_pair(s1, s2)
...
# ── Hook capture MLP output for SAE encoding ─────────────────
captured = {}
...
def get_sae_activation(s1, s2):
    ...
    dense = torch.zeros(flat.shape[0], num_latents, device=device,
                        dtype=torch.float32)
...
MODEL_NAME = f"DeepSeek-R1-Distill-Qwen-1.5B (MRPC, L{LAYER})"

fig, y_avg_p, n_avg_p = plot_avg_features(
    top_yes_idx[:5], "Paraphrase (semantic-sim)",
    yes_plot_acts, no_plot_acts, MODEL_NAME)
out = os.path.join(OUT_DIR, "firing_pattern_deepseek_mrpc_yes_avg5.png")
...
fig, y_avg_n, n_avg_n = plot_avg_features(
    ...
    yes_plot_acts, no_plot_acts, MODEL_NAME)
out = os.path.join(OUT_DIR, "firing_pattern_deepseek_mrpc_no_avg5.png")
fig.savefig(out, dpi=150, bbox_inches='tight')
fig.savefig(out.replace('.png', '.pdf'), bbox_inches='tight')
plt.close()
print(f"Saved: {out}")
...
for tag, feat_ids, colname in [
    ...
    csv_path = os.path.join(OUT_DIR, f"firing_values_deepseek_mrpc_{tag}_avg5.csv")
...
for tag, sel, plt_lst in [("yes", yes_select, yes_plot),
                           ("no",  no_select,  no_plot)]:
    csv_path = os.path.join(OUT_DIR, f"filtered_pairs_deepseek_mrpc_{tag}.csv")
    with open(csv_path, 'w', newline='') as f:
        w = csv.writer(f)
...
print(f"\n{'='*70}")
print(f"VALUES: Avg Top-5 Paraphrase (semantic-sim) (DeepSeek-R1-1.5B, MRPC, L{LAYER})")
...
print(f"\nVALUES: Avg Top-5 Non-paraphrase (DeepSeek-R1-1.5B, MRPC, L{LAYER})")
