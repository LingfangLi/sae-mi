# -*- coding: utf-8 -*-
"""Gemma-2B + Gemma Scope Attention SAE (SST-2) — Original Method

Adapted from the original Kaggle notebook for HPC cluster.
Pipeline: Baseline -> Layer-wise probing (attn.hook_z) -> SAE feature analysis
- Model: google/gemma-2b (TransformerLens)
...
train_dataset = dataset["train"].select(range(5000))  # subsample for speed
...
with open(os.path.join(RESULTS_DIR, 'baseline_results.txt'), 'w') as f:
    f.write("="*70 + "\n")
    f.write("BASELINE - ZERO-SHOT PROMPTING (Gemma-2B Base)\n")
    f.write("="*70 + "\n\n")
...
for layer_num in range(model.cfg.n_layers):
    ...
    with open(os.path.join(RESULTS_DIR, f"layer_{layer_num}_complete_analysis.txt"), "w") as f:
        f.write("="*70 + "\n")
        f.write(f"LAYER {layer_num} - COMPLETE ANALYSIS (Gemma Scope Attention SAE)\n")
        f.write("="*70 + "\n\n")
...
print(
    ...
    f"{'Neg Feat':<10} {'Common':<10} {'Pos Only':<10} {'Neg Only':<10}"
)
print("-"*84)
...
with open(os.path.join(RESULTS_DIR, "final_summary.txt"), "w") as f:
    f.write("="*70 + "\n")
    f.write("FINAL SUMMARY - GEMMA SCOPE ATTENTION SAE LAYER-WISE ANALYSIS\n")
    f.write("="*70 + "\n\n")
    ...
    f.write("-"*70 + "\n")
    f.write(
        f"{'Layer':<6} {'Acc%':<8} {'Improv':<10} {'Pos Feat':<10} "
        f"{'Neg Feat':<10} {'Common':<10} {'Pos Only':<10} "
        f"{'Neg Only':<10}\n"
    )
    f.write("-"*84 + "\n")
    for layer_num in sorted(layer_performance.keys()):
    ...
    f.write("\n3. TOP K MOST FREQUENT FEATURES PER LAYER\n")
    f.write("-"*70 + "\n")
    for layer_num in sorted(sae_feature_stats.keys()):
        if "error" not in sae_feature_stats[layer_num]:
            stats = sae_feature_stats[layer_num]
            f.write(f"\nLayer {layer_num}:\n")
            f.write("  POSITIVE (top-k by frequency): ")
            f.write(
                " ".join(
                    f"F{feat}({cnt})" for feat, cnt in stats["topk_pos_by_frequency"]
                )
                + "\n"
            )
            f.write("  NEGATIVE (top-k by frequency): ")
            f.write(
                " ".join(
                    f"F{feat}({cnt})" for feat, cnt in stats["topk_neg_by_frequency"]
                )
                + "\n"
            )
            f.write(
                f"  Common in top-k: {len(stats['topk_common_ids'])} features\n"
            )
