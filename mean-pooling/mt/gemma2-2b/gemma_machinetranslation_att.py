# -*- coding: utf-8 -*-
"""Gemma-2B + Gemma Scope Attention SAE (Machine Translation - Europarl)
...
- Dataset: Europarl v7 EN-FR (balanced: 1000 real + 1000 shuffled train, 500+500 val)
"""
...
# ================================================================
# 3. Load Europarl and build train/val sets
# ================================================================
DATA_DIR = "/mnt/scratch/users/sglli24/sae-mi/data"
...
train_dataset = [(f"{en} {SEP} {fr}", 1) for en, fr in pos_train] + \
                [(f"{en} {SEP} {fr}", 0) for en, fr in neg_train]
val_dataset = [(f"{en} {SEP} {fr}", 1) for en, fr in pos_val] + \
              [(f"{en} {SEP} {fr}", 0) for en, fr in neg_val]
...
print(f"Train: {len(train_texts)} (1000 real + 1000 shuffled)")
print(f"Val: {len(val_texts)} (500 real + 500 shuffled)")

# ================================================================
# 4. STEP 1: Baseline - Zero-Shot Prompting
# ================================================================
print("\n" + "="*70)
...
with torch.no_grad():
    for idx in tqdm(range(len(val_texts)), desc="Zero-Shot Evaluation"):
        sentence = val_texts[idx]
...
with open(os.path.join(RESULTS_DIR, 'baseline_results.txt'), 'w') as f:
    f.write("="*70 + "\n")
    f.write("BASELINE - ZERO-SHOT PROMPTING (Gemma-2B Base, Europarl MT)\n")
    f.write("="*70 + "\n\n")
...
# ================================================================
# 5. STEP 2: Layer-wise Analysis (Probes + Gemma Scope Attention SAEs)
# ================================================================
print("\n" + "="*70)
...
for layer_num in range(model.cfg.n_layers):
    ...
    with torch.no_grad():
        for idx in tqdm(range(len(train_texts)), desc=f"Train L{layer_num}"):
            sentence = train_texts[idx]
    ...
    with torch.no_grad():
        for idx in tqdm(range(len(val_texts)), desc=f"Val L{layer_num}"):
            sentence = val_texts[idx]
    ...
    with open(os.path.join(RESULTS_DIR, f"layer_{layer_num}_complete_analysis.txt"), "w") as f:
        f.write("="*70 + "\n")
        f.write(f"LAYER {layer_num} - COMPLETE ANALYSIS (Gemma Scope Attention SAE, Europarl MT)\n")
        f.write("="*70 + "\n\n")
        ...
        f.write(f"UNIQUE to EQUIVALENT only: {len(pos_only_features)}\n")
        f.write(f"UNIQUE to DIVERGENT only: {len(neg_only_features)}\n\n")
        ...
        f.write(f"\nG. TOP {TOP_K} MOST FREQUENT FEATURES (EQUIVALENT)\n")
        f.write("-"*70 + "\n")
        ...
        f.write(f"\nH. TOP {TOP_K} MOST FREQUENT FEATURES (DIVERGENT)\n")
        f.write("-"*70 + "\n")
        ...
        f.write(
            f"\nI. COMMON FEATURES AMONG TOP-{TOP_K} EQUIVALENT & DIVERGENT\n"
        )
        f.write("-"*70 + "\n")
...
print(
    f"\n{'Layer':<6} {'Acc%':<8} {'Improv':<10} {'Equiv Feat':<12} "
    f"{'Div Feat':<12} {'Common':<10} {'Equiv Only':<12} {'Div Only':<12}"
)
print("-"*94)
...
with open(os.path.join(RESULTS_DIR, "final_summary.txt"), "w") as f:
    f.write("="*70 + "\n")
    f.write("FINAL SUMMARY - GEMMA SCOPE ATTENTION SAE LAYER-WISE ANALYSIS (Europarl MT)\n")
    f.write("="*70 + "\n\n")
    ...
    f.write("-"*70 + "\n")
    f.write(
        f"{'Layer':<6} {'Acc%':<8} {'Improv':<10} {'Equiv Feat':<12} "
        f"{'Div Feat':<12} {'Common':<10} {'Equiv Only':<12} "
        f"{'Div Only':<12}\n"
    )
    f.write("-"*94 + "\n")
    for layer_num in sorted(layer_performance.keys()):
    ...
    f.write("- Higher accuracy = more translation quality information in that layer\n")
    f.write("- Many equivalent-only / divergent-only SAE features = stronger class separation\n")
