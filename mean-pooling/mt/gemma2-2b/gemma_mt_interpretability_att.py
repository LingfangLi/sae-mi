# -*- coding: utf-8 -*-
"""Gemma-2B Attention SAE - Europarl MT Interpretability Score (Original Method)
...
def chat_with_llama(prompt, model="llama-3.1-8b-instant", max_tokens=150):
    for attempt in range(10):
        try:
            ...
            time.sleep(wait)
    raise RuntimeError("Groq API failed after 10 retries")


# ================================================================
# 3. Load Gemma-2B & Europarl Validation
# ================================================================
...
# Build balanced val set (same seed as Stage 1)
random.seed(42)
...
SEP = "<|endoftext|>"
val_sentences = [f"{en} {SEP} {fr}" for en, fr in pos_val] + \
                [f"{en} {SEP} {fr}" for en, fr in neg_val]
...
def llama_interpretation(top_texts, top_acts):
    prompt = (
        "You are analyzing a sparse autoencoder feature from Gemma-2B on Europarl translation quality detection.\n"
    ...
    for i, (txt, acts) in enumerate(zip(top_texts, top_acts), 1):
        # Truncate activations to top-20 values to stay within Groq TPM limits
        top_indices = np.argsort(acts)[-20:][::-1]
        top_vals = [(int(idx), round(float(acts[idx]), 3)) for idx in top_indices if acts[idx] > 0]
        prompt += f"{i}. \"{txt[:100]}\"\nTop activations (pos, val): {top_vals}\n\n"
...
for layer in LAYERS_TO_ANALYZE:
    ...
    with open(report_file, "w", encoding="utf-8") as f:
        f.write(f"GEMMA-2B INTERPRETABILITY REPORT - Layer {layer}, Europarl MT, top-{TOP_K} global features\n")
        f.write("=" * 70 + "\n\n")
