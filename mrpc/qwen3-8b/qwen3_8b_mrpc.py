# -*- coding: utf-8 -*-
"""Qwen3-8B-Base + Qwen-Scope Residual SAE (MRPC Paraphrase Detection)

Stage 1: Zero-shot baseline + Layer-wise probing + SAE feature analysis (max-pool).
Same structure as sst2/qwen3-8b/qwen3_8b_pretrained.py, differences:
  - Dataset: GLUE MRPC (sentence-pair classification, paraphrase yes/no)
  - Input: f"{sentence1} {EOS} {sentence2}" (both sides concatenated)
  - Baseline prompt: paraphrase question, yes/no answer
...
dataset = load_dataset("nyu-mll/glue", "mrpc", cache_dir=CACHE_DIR)
...
def combine(ex):
    return f"{ex['sentence1']} {EOS} {ex['sentence2']}"


train_texts = [combine(ex) for ex in train_dataset]
val_texts = [combine(ex) for ex in val_dataset]
train_labels_list = list(train_dataset["label"])
val_labels_list = list(val_dataset["label"])
print(f"Train pairs: {len(train_texts)} | Val pairs: {len(val_texts)}")
...
# ================================================================
# 4. STEP 1: Zero-shot baseline (yes/no paraphrase)
# ================================================================
print("\n" + "=" * 70)
print("STEP 1: BASELINE — ZERO-SHOT PROMPTING (Qwen3-8B-Base, MRPC)")
print("=" * 70)
...
with open(os.path.join(RESULTS_DIR, "baseline_results.txt"), "w") as f:
    f.write("=" * 70 + "\n")
    f.write(f"BASELINE — MRPC ZERO-SHOT ({BASE_MODEL_ID})\n")
    f.write("=" * 70 + "\n\n")
...
def cache_pooled_reps(texts, labels, name):
    ...
    One forward per sample; mean-pool residual per layer (for LR probe).
    """
...
for layer_num in layers_to_run:
    ...
    # ---------- Part B: SAE feature stats (max-pool) ----------
    if args.skip_sae:
        continue
    ...
    with torch.no_grad():
        for idx in tqdm(range(len(val_texts)), desc=f"SAE   L{layer_num}"):
            ...
            layer_h = get_layer_rep(pair, layer_num)                # (1, seq, d_model)
            sae_feats = sae.encode(layer_h.to(sae.dtype))            # (1, seq, d_sae)
            # MAX-pool over sequence — matches paper §3.2
    ...
    with open(os.path.join(RESULTS_DIR, f"layer_{layer_num}_complete_analysis.txt"), "w") as f:
        f.write(f"LAYER {layer_num} — {BASE_MODEL_ID} + {SAE_RELEASE}/{sae_id} (MRPC)\n")
        f.write("=" * 70 + "\n\n")
...
with open(os.path.join(RESULTS_DIR, f"{summary_stem}.txt"), "w") as f:
    f.write("=" * 70 + "\n")
    f.write(f"FINAL SUMMARY — MRPC — {BASE_MODEL_ID} + {SAE_RELEASE}\n")
    f.write("=" * 70 + "\n\n")
