# -*- coding: utf-8 -*-
"""Qwen3-8B-Base + Qwen-Scope Residual SAE — MRPC Interpretability Score (max-pool)

Old single-file style (mirrors sst2/qwen3-8b/qwen3_8b_interpretability.py) but:
  - Task: MRPC paraphrase detection (sentence-pair)
  - Input: f"{sentence1} {EOS} {sentence2}"  (both concatenated)
  - Same max-pool ranking + top-N exemplars + Llama-3 explanation + held-out Pearson

Stage-2 script: run AFTER qwen3_8b_mrpc.py has produced probe accuracies.
Set --layers to the 2-3 best layers via CLI.
...
parser.add_argument("--val_n", type=int, default=408)      # MRPC val = 408
...
print("\nLoading MRPC val ...")
val_ds = load_dataset("nyu-mll/glue", "mrpc", split="validation", cache_dir=CACHE_DIR)
# Note: exemplar text shown to Llama = concatenated pair
val_sentences = [f"{ex['sentence1']} {EOS} {ex['sentence2']}"
                 for ex in val_ds][: args.val_n]
print(f"Using {len(val_sentences)} val pairs")
...
def llama_interpretation(top_texts, top_acts):
    prompt = (
        ...
        "paraphrase detection data (MRPC).\n"
        "Each item below is a concatenated sentence pair "
        "(sentence1 <|endoftext|> sentence2), accompanied by per-token activation "
        "strengths for this feature; larger numbers indicate stronger activation.\n\n"
        "From this data, give ONE concise sentence describing what pattern or "
        "linguistic concept this feature responds to (e.g. lexical overlap, "
        "semantic equivalence, numerical alignment, entity matching).\n\n"
    )
...
for layer in LAYERS_TO_ANALYZE:
    ...
    with open(report_file, "w", encoding="utf-8") as f:
        f.write(f"QWEN3-8B-BASE MRPC INTERPRETABILITY REPORT (max-pool)\n")
...
with open(summary_file, "w") as f:
    f.write("=" * 70 + "\n")
    f.write(f"OVERALL SUMMARY — Qwen3-8B MRPC interpretability (max-pool)\n")
