# -*- coding: utf-8 -*-
"""DeepSeek-R1-Distill-Qwen-1.5B + SAE (MRPC Paraphrase Detection)
...
# ================================================================
# 2. Load DeepSeek-R1-Distill-Qwen-1.5B and MRPC
# ================================================================
...
def combine_sentences(example):
    return f"{example['sentence1']} {EOS} {example['sentence2']}"

train_combined = [combine_sentences(ex) for ex in train_dataset]
...
with open(os.path.join(RESULTS_DIR, 'baseline_results_deepseek.txt'), 'w') as f:
    f.write("="*70 + "\n")
    f.write("BASELINE - ZERO-SHOT PROMPTING (DeepSeek-R1-Distill-Qwen-1.5B, MRPC)\n")
    f.write("="*70 + "\n\n")
...
for layer_num in range(n_layers):
    ...
    with open(os.path.join(RESULTS_DIR, f"layer_{layer_num}_complete_analysis_deepseek.txt"), "w") as f:
        f.write("="*70 + "\n")
        f.write(f"LAYER {layer_num} - COMPLETE ANALYSIS (DeepSeek-R1-Distill-Qwen-1.5B, MRPC)\n")
        f.write("="*70 + "\n\n")
...
print(
    ...
    f"{'Para Feat':<12} {'NonP Feat':<12} {'Common':<10}"
)
print("-"*90)
...
with open(os.path.join(RESULTS_DIR, "final_summary_deepseek.txt"), "w") as f:
    f.write("="*70 + "\n")
    f.write("FINAL SUMMARY - LAYER-WISE ANALYSIS (DeepSeek-R1-Distill-Qwen-1.5B + SAE, MRPC)\n")
    f.write("="*70 + "\n\n")
